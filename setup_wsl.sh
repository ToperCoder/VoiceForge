...
# Target: RTX 50xx (Blackwell, SM120), CUDA 12.8, Debian 12 Bookworm
set -e
...
# 2. CUDA Toolkit 12.8 via network installer
echo "[2/5] Setting up CUDA Toolkit 12.8..."
if ! command -v nvcc &> /dev/null; then
    echo "  Adding NVIDIA CUDA network repository (wsl-ubuntu)..."
    wget -q https://developer.download.nvidia.com/compute/cuda/repos/wsl-ubuntu/x86_64/cuda-keyring_1.1-1_all.deb
    sudo dpkg -i cuda-keyring_1.1-1_all.deb
    rm cuda-keyring_1.1-1_all.deb
    sudo apt-get update
    sudo apt-get install -y cuda-toolkit-12-8
    echo "  Appending CUDA paths to ~/.bashrc..."
